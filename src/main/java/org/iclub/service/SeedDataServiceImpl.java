...
public class SeedDataServiceImpl implements SeedDataService {
    ...
    public SeedDataServiceImpl(UserService userService, SettingService settingService,
            BinaryFileService binaryFileService, ResourceLoader resourceLoader) {
    ...
    public boolean init() {
        ...
        if (createSetting(SettingService.FACEBOOK, "facebook")) {
            updated = true;
        }
        if (createSetting(SettingService.TWITTER, "Twitter")) {
            updated = true;
        }
        if (createSetting(SettingService.TITLE, "Default")) {
            updated = true;
        }
        if (createSetting(SettingService.DESCRIPTION, "This is the default descrption of the club.")) {
            updated = true;
        }
        if (createSetting(SettingService.PINTEREST, "pinterest")) {
            updated = true;
        }
        if (createSetting(SettingService.YOUTUBE, "youtube")) {
            updated = true;
        }
        ...
        if (!optional.isPresent()) {
            try {
                binaryFileService
                        .save(BinaryFile.getBinaryFile("/images/logo_default.png", "image/png", true, false, false, resourceLoader));
        ...
        if (scrollers == null || scrollers.size() == 0) {
            try {
                binaryFileService
                        .save(BinaryFile.getBinaryFile("/images/cycling.jpg", "image/png", false, true, false, resourceLoader));
                binaryFileService
                        .save(BinaryFile.getBinaryFile("/images/forest-runner.jpg", "image/jpeg", false, true, false, resourceLoader));
                binaryFileService.save(
                        BinaryFile.getBinaryFile("/images/mountain-bikers.jpg", "image/jpeg", false, true, false, resourceLoader));
                binaryFileService
                        .save(BinaryFile.getBinaryFile("/images/open_water.jpg", "image/jpeg", false, true, false, resourceLoader));
                binaryFileService
                        .save(BinaryFile.getBinaryFile("/images/trail-runners.jpg", "image/jpeg", false, true, false, resourceLoader));
